...
async def mes_new_game(message: Message):
    ...
    for game in config.games:
        ...
        if coin:
            ...
            await message.answer(text = f'{message.from_user.first_name}, не расстраивайся! Первый ход '
                                        f'делает Бот.')
    ...
    if message.text.isdigit():
        if 0 < int(message.text) < 29:
            ...
            await message.answer(text = f'Ах ты, хитрый {message.from_user.first_name}!'
                                        f' Конфет надо взять хотя бы одну, но не больше 28. Попробуй еще раз!')
    else:
...
async def bot_turn(message: Message):
    ...
    config.games[message.from_user.id] = config.games.get(message.from_user.id) - take_amount
    name = message.from_user.first_name
    await message.answer(text = f'Бот взял {take_amount} кофет и на столе осталось {config.games.get(message.from_user.id)}.')
